...
while True:
        ...
        for chan in chanindx:
            ...
            for sample in range(len(frequency)):
                if frequency[sample]>=lo and frequency[sample]<=hi:
                    ...
                    band_bins.append(abs(F[sample][chan]*F[sample][chan]))
                    ii.append(sample)
                    count    += 1
            try:
    ...
    i = 0
    power_rel = []
    for j, band in enumerate(bandname):
        for k, chan in enumerate(channame):
            power_rel.append(power[i]/(np.sum(power[0+k:len(power):len(channame)])))
            i+=1
    i = 0
    for band in bandname:
        for chanix, chan in enumerate(channame):
            key = "%s.%s.%s" % (chan, prefix, band)
            key_peaks = "%s.%s.%s" % (chan, prefix_peaks, band)
            patch.setvalue(key, power_rel[i])
