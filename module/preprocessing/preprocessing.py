...
>>>>>>> origin
    ...
    averaging = True
    if not(averaging is None):
        dat_output = np.array([np.mean(dat_output[:,[1,2]], axis=1), np.mean(dat_output[:,[0,3]], axis=1), dat_output[:,4]]).T
        print(type(dat_output), dat_output.shape)
